...
## Multi-IDE Support: OpenCode + Claude Code Compatibility

**Fecha**: 2026-02-22

### Nuevo Feature

Soporte dual para múltiples IDEs con MCP (Model Context Protocol):

| IDE | Método de Activación | Variable/Arg |
|-----|---------------------|--------------|
| **Claude Code** | Argumento | `--debug-terminal` |
| **OpenCode** | Variable de Entorno | `OMNYSYS_DEBUG_TERMINAL=1` |

### Cambios en `mcp-server.js`

```javascript
// Ahora acepta ambos métodos
const debugTerminal = process.argv.includes('--debug-terminal') || 
                     process.env.OMNYSYS_DEBUG_TERMINAL === '1';
```
...
```json
{
  "mcp": {
    "omny-system": {
      "command": ["node", "mcp-server.js", "project-path"],
      "environment": {
        "OMNYSYS_DEBUG_TERMINAL": "1"
      }
    }
  }
}
...
```json
{
  "mcpServers": {
    "omny-system": {
      "command": ["node", "mcp-server.js", "project-path", "--debug-terminal"]
    }
  }
}
```
...
### Benefits
- ✅ Ventana de logs visible en ambos IDEs
- ✅ Debug en tiempo real sin depender de archivos
