# OmnySys v0.5.1 - Bug Fixes & MCP Optimization

**Fecha:** 2026-02-06  
**Tipo:** Bug Fix + Performance Optimization  
**Estado:** ✅ Complete

---

## 🐛 **BUGS CRÍTICOS CORREGIDOS**

### **1. Bloqueo del Event Loop durante Inicialización**
**Problema:** El sistema bloqueaba la terminal durante minutos al iniciar el MCP debido a procesamiento secuencial.

**Archivos Modificados:**
- `src/core/orchestrator/lifecycle.js` - Inicialización no bloqueante
- `src/core/orchestrator/llm-analysis.js` - Procesamiento paralelo por batches (10 archivos)
- `src/layer-a-static/pipeline/parse.js` - Parseo en paralelo por batches (20 archivos)
- `src/core/file-watcher/lifecycle.js` - Yield cada 50 archivos con `setImmediate()`

**Solución:**
```javascript
// ANTES (bloqueante):
for (const file of files) {
  await processFile(file); // Bloquea cada iteración
}

// DESPUÉS (no bloqueante):
for (let i = 0; i < files.length; i += BATCH_SIZE) {
  const batch = files.slice(i, i + BATCH_SIZE);
  await Promise.all(batch.map(f => processFile(f)));
  await new Promise(resolve => setImmediate(resolve)); // Yield
}
```

### **2. Inconsistencia de Rutas (`.OmnySysData` vs `.omnysysdata`)**
**Problema:** El código usaba diferentes nombres de directorio, causando "datos no encontrados".

**Archivos Modificados:**
- `src/layer-c-memory/mcp-server.js` - 5 referencias corregidas
- `src/layer-c-memory/omnysysdata-generator.js` - Constante `OMNYSYSDATA_DIR`
- `src/layer-c-memory/populate-omnysysdata.js` - Rutas unificadas
- `src/layer-a-static/storage/storage-manager.js` - `DATA_DIR`

**Solución:** Estandarizado todo a `.omnysysdata` (minúsculas con punto).

### **3. Inicio Lento del MCP (30s → 250ms)**
**Problema:** El MCP repopulaba y recacheaba 433 archivos en CADA inicio.

**Cambios en `mcp-server.js`:**
- Lazy loading: Solo carga el índice al inicio
- Datos se cargan on-demand cuando se necesitan
- Eliminado cacheo upfront de metadata, conexiones y assessments

**Resultado:**
- **Antes:** 30+ segundos (repopulaba todo)
- **Ahora:** 247 milisegundos (solo índice)

---

## ⚡ **OPTIMIZACIONES DE PERFORMANCE**

### **Batch Processing**
- Procesamiento paralelo con `Promise.all()`
- Batches de 10 archivos para LLM analysis
- Batches de 20 archivos para parsing
- Yield al event loop entre batches

### **Lazy Loading**
- Cache inicializado vacío
- Datos se cargan bajo demanda
- Metadata del índice solo carga contadores básicos

---

## 🔧 **CONFIGURACIÓN MCP PARA OPENCODE**

**Archivo Creado:** `opencode.json`

```json
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "omny-system": {
      "type": "local",
      "command": ["node", "src/layer-c-memory/mcp-server.js", "."],
      "enabled": true,
      "timeout": 30000
    }
  }
}
```

**Estado:** Configurado pero requiere investigación de handshake con OpenCode.

---

## 📊 **ESTADÍSTICAS DEL SISTEMA**

- **Archivos Analizados:** 433
- **Funciones:** 943
- **Conexiones Semánticas:** 4002
- **Archivos de Alto Riesgo:** 1
- **Archivos de Riesgo Medio:** 75

**Tiempo de Inicio:**
- Orchestrator: ~1.5s
- Cache (lazy): ~250ms
- **Total:** < 2 segundos

---

## 🧪 **VERIFICACIÓN**

```bash
# Test de inicio rápido
node src/layer-c-memory/mcp-server.js .

# Resultado esperado:
# ✅ MCP Server Ready!
# Cache init time: ~250ms
# Using existing .omnysysdata/ (433 files)
```

---

## 📝 **NOTAS TÉCNICAS**

### **APIs Corregidas:**
- `this.cache.ramCacheSet()` → `this.cache.set()`
- `this.cache.getCacheStats()` → `this.cache.getRamStats()`

### **Event Loop Yield:**
```javascript
// Patrón aplicado en 4 archivos críticos
await new Promise(resolve => setImmediate(resolve));
```

### **Métodos de Inicialización No-Bloqueantes:**
```javascript
// ANTES:
await this._analyzeComplexFilesWithLLM();

// DESPUÉS:
this._analyzeComplexFilesWithLLM().then(() => {
  console.log('✅ LLM analysis queue ready');
}).catch(err => {
  console.error('❌ LLM analysis setup failed:', err.message);
});
```

---

## 🔧 **FIXES ADICIONALES - Actualización de Imports Deprecados**

**Fecha:** 2026-02-06
**Estado:** ✅ Complete

### Archivos Modificados:
- `src/layer-a-static/query/export.js` - 🆕 Nuevo módulo creado
- `src/layer-a-static/query/index.js` - Agregado export de `exportFullSystemMapToFile`
- `src/cli/commands/export.js` - Actualizado import de deprecated path
- `src/layer-c-memory/export-system-map.js` - Verificado path correcto
- `src/layer-c-memory/mcp-server.js` - Actualizado imports deprecated
- `src/layer-c-memory/populate-omnysysdata.js` - Actualizado imports deprecated

### Cambios:
1. **Creada función `exportFullSystemMapToFile`** que estaba siendo importada pero no existía
2. **Eliminated deprecation warnings** actualizando paths a `query/index.js`
3. **Mejorada estructura de módulos** con exports proper desde `src/layer-a-static/query/`

### Nueva API:
```javascript
import { exportFullSystemMapToFile } from '../layer-a-static/query/index.js';

const result = await exportFullSystemMapToFile(projectPath, outputPath);
// Returns: { filePath, sizeKB, filesExported }
```

---

## 🚀 **PRÓXIMOS PASOS**

1. Investigar handshake MCP con OpenCode
2. Probar integración completa con herramientas
3. Validar que las tools respondan en < 100ms

---

**Cambios Totales:** 10 archivos modificados  
**Líneas Añadidas:** ~150  
**Líneas Eliminadas:** ~200  
**Bugs Críticos Corregidos:** 3  
**Performance Improvement:** 120x más rápido (30s → 250ms)
