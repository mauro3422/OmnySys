...
- Nombres reales → ayudar al developer, navegar codigo
- Version estandarizada → pattern matching, training de IA, deteccion cross-project
...
### Principio Fundamental: CABLES, NO SEÑALES
...
// eval() es un componente OPACO pero con cables mapeados:
{
  ...
  dataFlow: {
    ...
    opaqueExecution: {
      ...
      // El cable existe, la señal es opaca
...
**Cobertura de cables**: ~90%. El cable esta mapeado. Solo la señal interna es opaca.
...
|------|-------|------------|---------|
| eval/dynamic code | 0% | ~90% | Cable mapeado (entrada→eval→salida) + test probes |
...
```javascript
{
  ...
  standardized: {
    pattern: "PROCESS_FUNC(ENTITY_1, ID_1) { CALC → READ → TRANSFORM → PERSIST → RETURN }",
...
```
          ...
          Cada ATOMO tiene:
          ├── Estructura (nombre, params, complexity)
          ├── Conexiones (calls, calledBy)
          ├── Side Effects (network, DOM, storage)
          ├── Archetype (god-function, hot-path, etc.)
          ├── Data Flow (inputs -> transformations -> outputs)  <- NUEVO
          ├── Semantic (verb, domain, entity)                   <- NUEVO
          ├── Standardized (patron universal)                   <- NUEVO
          └── Race Condition Risk                               <- NUEVO
