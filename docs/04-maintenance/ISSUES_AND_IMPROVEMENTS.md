...
| **Causa** | El bootstrap de `POST /mcp` no distingu�a bien entre una sesi�n stale, un initialize fr�o y una recuperaci�n persistida |
| **Estado** | ? RESUELTO |

**Fix aplicado**:
- `POST /mcp` ahora siempre parsea el body, incluso si existe un `mcp-session-id` viejo.
- Se agreg� metadata de handshake para distinguir `http-initialize`, `http-reinitialize` y `http-recovered`.
- La telemetr�a de sesi�n y tool runs ahora guarda `transport_origin`.

**Resultado**:
- El daemon puede estar sano y aun as� la sesi�n fallar si el contrato de init es ciego.
