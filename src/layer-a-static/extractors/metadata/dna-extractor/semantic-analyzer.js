...
/**
 * Regex patterns that match test framework generated names (Vitest, Jest, etc.)
 * These should be ignored as they are not real function names.
 */
const TEST_FRAMEWORK_GENERATED_NAME_PATTERNS = [
  /^each_/,  // Vitest .each() generated names
  /^\.each/,  // Vitest .each() alternative format
  /_arg\d+$/,  // Test framework argument placeholders (it_arg1, describe_arg2, etc.)
  /^test_each_/,  // Jest/Vitest test.each pattern
  /^describe_each_/,  // Jest/Vitest describe.each pattern
  /^it_each_/,  // Jest/Vitest it.each pattern
];

/**
 * Detects if a function name is generated by a test framework (.each, dynamic tests, etc.)
 * @param {string} name - Function name to check
 * @returns {boolean} True if name appears to be framework-generated
 */
function isTestFrameworkGeneratedName(name) {
  if (!name || typeof name !== 'string') return false;
  return TEST_FRAMEWORK_GENERATED_NAME_PATTERNS.some(pattern => pattern.test(name));
}
...
export function computeSemanticFingerprint(atom) {
  ...
  // Early return for test framework generated names (Vitest .each(), Jest dynamic tests, etc.)
  // These are not real function names and should not be used for duplicate detection
  if (isTestFrameworkGeneratedName(name)) {
    return 'test_framework:generated:skip:skip';
  }
