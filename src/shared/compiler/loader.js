...
import { buildFolderizationPropagationAdoptionTargets } from './folderization-propagation-adoption-targets.js';
import {
...
export async function loadCompilerExplainability(projectPath, watcherAlerts = [], sharedState = {}, watcherStats = null, folderizationOptions = {}) {
  try {
    ...
    let policySummary;
    if ((forceRescan || forceFresh) && existingExplainability?.policySummary) {
    ...
    const { RepositoryFactory, getRepository } = await import('#layer-c/storage/repository/index.js');
    if (forceFreshRepository) {
      // Fresh reads should not reuse a potentially stale singleton connection.
