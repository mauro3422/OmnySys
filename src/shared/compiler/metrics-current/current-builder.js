/**
 * @fileoverview Builds the current metrics snapshot object and summary text.
 *
 * Extracted from buildCurrentMetrics to reduce complexity and improve
 * testability. This module is responsible for assembling the final
 * current snapshot object from all the intermediate summaries.
 *
 * @module shared/compiler/metrics-current/current-builder
 */
...
function buildTelemetrySummaryFields(current) {
  const fields = [];

  if (current.clientSyncState && current.clientSyncState !== 'fresh') {
    fields.push(`clientsync=${current.clientSyncState}`);
  }
  if (current.transportProvenanceState && current.transportProvenanceState !== 'fresh') {
    fields.push(`transport=${current.transportProvenanceState}`);
  }
  if (current.transportAlertState && current.transportAlertState !== 'fresh') {
    fields.push(`transportAlerts=${current.transportAlertState}`);
  }
  if (current.requestDeliveryState && current.requestDeliveryState !== 'fresh') {
    fields.push(`delivery=${current.requestDeliveryState}`);
  }
  if (current.topologyState && current.topologyState !== 'fresh') {
    fields.push(`topology=${current.topologyState}`);
  }
  ...
  if (current.startupTelemetry?.state) {
    fields.push(`startup=${current.startupTelemetry.state}:${Math.round(current.startupTelemetry.totalDurationMs || 0)}ms`);
  }

  if (current.folderizationNormalization?.recommendedAction) {
    fields.push(`normalize=${current.folderizationNormalization.recommendedAction}:${current.folderizationNormalization.renameTargetCount}`);
  }

  if (current.folderizationAutomation?.propagationAdoption?.adoptionState) {
    fields.push(`adoption=${current.folderizationAutomation.propagationAdoption.adoptionState}:${current.folderizationAutomation.propagationAdoption.missingSystemCount || 0}`);
  }

  if (current.proxyRuntimeTelemetry?.state) {
    fields.push(`proxy=${current.proxyRuntimeTelemetry.state}`);
  }
  if (current.bridgeRuntimeTelemetry?.state) {
    fields.push(`bridge=${current.bridgeRuntimeTelemetry.state}`);
  }

  if (current.toolTelemetry?.pressureRuns > 0) {
    fields.push(`repair=${current.toolTelemetry.repairedRuns}/${current.toolTelemetry.pressureRuns}`);
  }

  fields.push(`issues=${current.issueCount}`);

  if (current.watcherIssuePersistence) {
    fields.push(`issuePersistence=${current.watcherIssuePersistence.activeIssueCount || 0}:${current.watcherIssuePersistence.orphanedIssues || 0}/${current.watcherIssuePersistence.withoutLifecycle || 0}/${current.watcherIssuePersistence.withoutContext || 0}`);
  }

  fields.push(`dups=${current.structuralGroups + current.conceptualGroups}`);
  fields.push(`folder=${current.alreadyFolderizedFamilies}/${current.flatFamilies + current.mixedFamilies + current.alreadyFolderizedFamilies}`);

  if (current.canonicalPromotion?.promotionState) {
    fields.push(`promotion=${current.canonicalPromotion.promotionState}:${current.canonicalPromotion.candidateCount || 0}`);
  }
  if (current.policyCoverage?.coverageState) {
    fields.push(`policy=${current.policyCoverage.coverageState}:${current.policyCoverage.coverageScore || 0}`);
  }
  if (current.propagationLedger?.state) {
    fields.push(`ledger=${current.propagationLedger.state}:${current.propagationLedger.policyDriftCount || 0}`);
  }
  if (current.metricAlignment?.state) {
    fields.push(`alignment=${current.metricAlignment.state}`);
  }
  if (current.folderizationPropagation?.decision) {
    fields.push(`propagation=${current.folderizationPropagation.decision}`);
  }
  if (current.semanticGranularitySummary?.state) {
    fields.push(`semantic=${current.semanticGranularitySummary.state}:${current.semanticGranularitySummary.fileLevelTotal}f/${current.semanticGranularitySummary.atomLevelTotal}a`);
  }

  fields.push(`coverage=${Math.round(current.liveCoverageRatio * 100)}%`);

  return fields;
}

function buildSummaryText(current) {
  const baseFields = [
  ...
  const telemetryFields = buildTelemetrySummaryFields(current);

  return [...baseFields, ...telemetryFields].join(' | ');
}

export function buildCurrentSnapshotObject({
  projectPath,
  scopePath,
  focusPath,
  captureSource,
  ...
  const current = {
    ...
    // MCP session transport provenance
    ...
    topologyStateCount: topologySummary?.stateCounts || null,
    ...
    // System inventory and promotion
  ...
  current.summaryText = buildSummaryText(current);

  return current;
}

export {
  compactFolderizationPropagation,
  compactFolderizationNormalization
};
