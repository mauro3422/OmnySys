...
// ── Section builders (extracted to reduce CC of main function) ──────────

function buildArchiveDaily(current, folderizationPropagation, policyCoverage, trend) {
  return {
    capturedAt: current.capturedAt || current.capturedAt || null,
...
function buildArchiveLifetime(ha) {
  return {
...
function buildHealthSection(current, cpContracts, folderizationPropagation, policyCoverage, propagationExpansion) {
  return {
    globalHealthScore: asNumber(current.globalHealthScore, asNumber(current.healthScore, 0)),
    globalHealthGrade: current.globalHealthGrade || current.healthGrade || 'F',
    healthScore: asNumber(current.healthScore, 0),
...
function buildMetricsSection(current, cpContracts, pipelineTimingTelemetry) {
  return {
    issueCount: asNumber(current.issueCount, 0),
...
function buildSessionsSection(summary) {
...
export function buildCompilerHealthDashboard(snapshot = null, compilerExplainability = null, options = {}) {
  ...
  const archiveDaily = healthArchive
    ? buildArchiveDaily(current, folderizationPropagation, policyCoverage, trend)
    : null;
  ...
  return {
    ...
    lifetime: buildLifetime(normalized.healthArchive),
