...
export function buildToolRunTelemetrySummary(db, options = {}) {
  ...
  try {
    ...
    const groupedRows = db.prepare(`
      SELECT
        tool_name,
        COUNT(*) as run_count,
        SUM(CASE WHEN success = 1 THEN 1 ELSE 0 END) as success_count,
        SUM(CASE WHEN repair_status = 'repaired' THEN 1 ELSE 0 END) as repaired_count,
        SUM(CASE WHEN repair_status = 'thrashing' THEN 1 ELSE 0 END) as thrashing_count,
        SUM(CASE WHEN repair_status = 'stable' THEN 1 ELSE 0 END) as stable_count,
        SUM(CASE WHEN before_watcher_alert_count > 0 OR before_recent_error_count > 0 OR before_recent_warning_count > 0 THEN 1 ELSE 0 END) as pressure_count,
        SUM(CASE WHEN after_snapshot_json IS NOT NULL OR after_notifications_json IS NOT NULL THEN 1 ELSE 0 END) as observation_count,
        SUM(CASE WHEN alert_clearance > 0 OR error_clearance > 0 OR warning_clearance > 0 THEN 1 ELSE 0 END) as clearance_count,
        AVG(repair_score) as avg_repair_score,
        AVG(duration_ms) as avg_duration_ms,
        MAX(ended_at) as last_run_at
      FROM mcp_tool_runs
    ...
    const noiseSummary = buildToolTelemetryNoiseSummary(groupedRows, { totalRuns });
    const cachePolicySummary = buildToolCachePolicySummary(groupedRows, { totalRuns, noiseSummary });
    const topTools = groupedRows
      .slice(0, 5)
      .map((row) => {
        const noise = classifyToolTelemetryNoise({
          toolName: row.tool_name,
          runCount: row.run_count,
          successCount: row.success_count,
          repairedCount: row.repaired_count,
          thrashingCount: row.thrashing_count,
          stableCount: row.stable_count,
          pressureCount: row.pressure_count,
          observationCount: row.observation_count,
          clearanceCount: row.clearance_count,
          avgDurationMs: row.avg_duration_ms,
          avgRepairScore: row.avg_repair_score
        });
        const cachePolicy = buildToolCachePolicySummary([{
          toolName: row.tool_name,
          runCount: row.run_count,
        ...
        return {
          ...
          noise,
          cachePolicy: cachePolicy.topTools[0] || null
        };
      });
