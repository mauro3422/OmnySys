...
function buildSnapshotsRow(daily, lifetime) {
  return {
    area: 'Snapshots',
    state: daily && lifetime ? 'daily + lifetime' : daily ? 'daily' : lifetime ? 'lifetime' : 'live only',
    detail: daily
...
function buildNamingDriftRow(namingDrift) {
  return {
    area: 'Naming Drift',
    state: namingDrift?.state || 'missing',
    detail: namingDrift
      ? `score=${normalizeCount(namingDrift.score || 0)} | reason=${namingDrift.reason || 'n/a'} | next=${namingDrift.recommendation || 'n/a'}`
      : 'folderization naming drift signal not available',
...
export function buildStatusTableRows(status = {}) {
  const ctx = extractStatusContext(status);
  return [
    buildDaemonRow(status),
    buildDatabaseRow(ctx.databaseHealth, ctx.healthScore, ctx.healthGrade),
    buildSnapshotsRow(ctx.daily, ctx.lifetime),
    ...
    buildAduanaRow(ctx.policyCoverage, ctx.controlPlaneContracts, ctx.metricAlignment),
    buildPromotionRow(ctx.controlPlaneContracts),
    buildAutomationRow(ctx.folderizationAutomation),
    buildAdoptionRow(ctx.folderizationAdoption),
    buildNamingDriftRow(ctx.namingDrift),
    buildCacheRow(status),
    buildWatcherRow(ctx.watcher),
    buildErrorsRow(ctx.recentErrorSummary)
  ].filter(Boolean);
}
