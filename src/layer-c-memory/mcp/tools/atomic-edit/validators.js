/**
 * @fileoverview Validaciones pre y post edit
 */

import path from 'path';
import { createLogger } from '../../../../utils/logger.js';
import { loadAtoms } from '#layer-c/storage/index.js';
import { checkImportExists } from '../validate-imports.js';
import { extractImportsFromCode } from './exports.js';
import { findCallersEfficient } from './search.js';

const logger = createLogger('OmnySys:atomic:validators');

/**
 * Valida que todos los imports en el nuevo código existan
 */
export async function validateImportsInEdit(filePath, newString, projectPath) {
  const imports = extractImportsFromCode(newString);
  const brokenImports = [];
  
  for (const importPath of imports) {
    if (importPath.startsWith('.') || importPath.startsWith('#')) {
      const check = await checkImportExists(importPath, filePath, projectPath);
      if (!check.exists) {
        brokenImports.push({
          import: importPath,
          attemptedPaths: check.attemptedPaths,
          suggestion: `Verificar que el módulo "${importPath}" exista. Intentado: ${check.attemptedPaths.slice(0, 3).join(', ')}`
        });
      }
    }
  }
  
  return brokenImports;
}

/**
 * Valida post-edit de forma OPTIMIZADA
 * Solo analiza callers de funciones modificadas
 */
export async function validatePostEditOptimized(filePath, projectPath, previousAtoms, currentAtoms) {
  const result = {
    valid: true,
    errors: [],
    warnings: [],
    brokenCallers: [],
    affectedFiles: 0,
    debugInfo: {
      currentAtomsCount: currentAtoms.length,
      previousAtomsCount: previousAtoms.length,
      atoms: []
    }
  };
  
  try {
    const modifiedAtoms = currentAtoms.filter(current => {
      const previous = previousAtoms.find(p => p.name === current.name);
      if (!previous) return false;
      
      const currentRequired = (current.signature?.params || []).filter(p => !p.optional).length;
      const previousRequired = (previous.signature?.params || []).filter(p => !p.optional).length;
      
      return currentRequired !== previousRequired;
    });
    
    if (modifiedAtoms.length === 0) {
      logger.info('[PostEditOptimized] No signature changes detected');
      return result;
    }
    
    logger.info(`[PostEditOptimized] Checking ${modifiedAtoms.length} modified functions`);
    
    for (const atom of modifiedAtoms) {
      const requiredParams = (atom.signature?.params || []).filter(p => !p.optional).length;
      
      const callers = await findCallersEfficient(atom.name, projectPath, filePath);
      
      logger.info(`[PostEditOptimized] ${atom.name}: ${callers.length} callers found`);
      
      for (const caller of callers) {
        if (caller.argumentCount !== undefined && caller.argumentCount < requiredParams) {
          result.valid = false;
          result.brokenCallers.push({
            caller: caller.name,
            file: caller.filePath,
            line: caller.line,
            reason: `Passes ${caller.argumentCount} args but ${atom.name} requires ${requiredParams}`,
            severity: 'critical'
          });
        }
      }
    }
    
    result.affectedFiles = new Set(result.brokenCallers.map(c => c.file)).size;
    
  } catch (error) {
    logger.error(`[PostEditOptimized] Error: ${error.message}`);
    result.errors.push(`Validation error: ${error.message}`);
  }
  
  return result;
}

/**
 * Valida post-edit: verifica que callers todavía puedan importar
 */
export async function validatePostEdit(filePath, projectPath, previousAtoms, currentAtoms) {
  const result = {
    valid: true,
    errors: [],
    warnings: [],
    brokenCallers: [],
    affectedFiles: 0,
    debugInfo: {
      currentAtomsCount: currentAtoms.length,
      previousAtomsCount: previousAtoms.length,
      atoms: []
    }
  };
  
  try {
    const absolutePath = path.isAbsolute(filePath) ? filePath : path.join(projectPath, filePath);
    const relativePath = path.relative(projectPath, absolutePath);
    const fileAtoms = await loadAtoms(projectPath, relativePath);
    
    const combinedAtoms = [...currentAtoms];
    
    for (const atom of fileAtoms) {
      if (!combinedAtoms.find(a => a.name === atom.name || a.id === atom.id)) {
        combinedAtoms.push(atom);
      }
    }
    
    for (const currentAtom of currentAtoms) {
      const previousAtom = previousAtoms.find(a => a.name === currentAtom.name);
      
      const atomDebug = {
        name: currentAtom.name,
        previousFound: !!previousAtom,
        previousParams: previousAtom?.signature?.params?.length || 0,
        currentParams: currentAtom.signature?.params?.length || 0,
        previousRequired: previousAtom ? (previousAtom.signature?.params || []).filter(p => !p.optional).length : 0,
        currentRequired: (currentAtom.signature?.params || []).filter(p => !p.optional).length
      };
      result.debugInfo.atoms.push(atomDebug);
      
      if (previousAtom && previousAtom.name !== currentAtom.name) {
        logger.info(`[PostEdit] Function renamed: ${previousAtom.name} -> ${currentAtom.name}`);
        
        const importers = combinedAtoms.filter(atom =>
          atom.id !== currentAtom.id &&
          atom.file !== currentAtom.file &&
          atom.calls?.some(call =>
            call.callee === previousAtom.name ||
            call.callee?.endsWith(`::${previousAtom.name}`)
          )
        );
        
        if (importers.length > 0) {
          result.valid = false;
          for (const importer of importers) {
            result.brokenCallers.push({
              caller: importer.name,
              file: importer.file,
              line: importer.calls?.find(c => 
                c.callee === previousAtom.name || 
                c.callee?.endsWith(`::${previousAtom.name}`)
              )?.line || 0,
              reason: `IMPORT BREAK: Function renamed from '${previousAtom.name}' to '${currentAtom.name}'`,
              severity: 'critical',
              oldName: previousAtom.name,
              newName: currentAtom.name
            });
          }
        }
      }
      
      const currentRequiredParams = (currentAtom.signature?.params || [])
        .filter(p => !p.optional).length;
      const previousRequiredParams = (previousAtom?.signature?.params || [])
        .filter(p => !p.optional).length;
      
      if (currentRequiredParams !== previousRequiredParams) {
        const callers = combinedAtoms.filter(atom => {
          if (!atom.calls || atom.calls.length === 0) return false;
          return atom.calls.some(call =>
            call.callee === currentAtom.name ||
            call.callee?.endsWith(`::${currentAtom.name}`)
          );
        });
        
        for (const caller of callers) {
          if (!caller.calls) continue;
          
          const callsToEdited = caller.calls.filter(call => 
            call.callee === currentAtom.name || 
            call.callee?.endsWith(`::${currentAtom.name}`)
          );
          
          for (const call of callsToEdited) {
            if (call.arguments?.some(arg => arg.isSpread)) continue;
            
            const argCount = call.argumentCount || 0;
            
            if (argCount < currentRequiredParams) {
              result.valid = false;
              result.brokenCallers.push({
                caller: caller.name,
                file: caller.filePath,
                line: call.line,
                reason: `BREAKING CHANGE: passes ${argCount} args but requires ${currentRequiredParams}`,
                severity: 'critical'
              });
            }
          }
        }
      }
    }
    
    result.affectedFiles = new Set(result.brokenCallers.map(c => c.file)).size;
    
    if (result.brokenCallers.length > 0) {
      result.errors.push(`Post-edit validation failed: ${result.brokenCallers.length} broken calls`);
    }
    
  } catch (error) {
    logger.error(`[PostEdit] Validation error: ${error.message}`);
    result.valid = false;
    result.errors.push(`Post-edit validation error: ${error.message}`);
  }
  
  return result;
}
