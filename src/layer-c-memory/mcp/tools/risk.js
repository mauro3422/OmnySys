...
function calculateFileRisk(atoms) {
  ...
  // Factor 5: ÁLGEBRA DE GRAFOS - Unificado con AnalysisEngine
  const hubs = atoms.filter(a => {
    const inDegree = a.graph?.inDegree || 0;
    const outDegree = a.graph?.outDegree || 0;
    return AnalysisEngine.classifyNode(inDegree, outDegree) === NODE_TYPES.HUB;
  });
...
export async function get_risk_assessment(args, context) {
  ...
  try {
    ...
    // Enriquecer con centrality, propagation, risk
    ...
    if (!allAtoms || allAtoms.length === 0) {
      return {
        summary: { totalFiles: 0, totalIssues: 0, criticalCount: 0, highCount: 0, mediumCount: 0, lowCount: 0, tunnelVisionIntegrated: true },
        topRiskFiles: [],
    ...
    for (const atom of allAtoms) {
      ...
      if (!atomsByFile[filePath]) atomsByFile[filePath] = [];
    ...
    for (const [filePath, atoms] of Object.entries(atomsByFile)) {
      ...
      if (risk.factors.length > 0) {
        fileRisks.push({
          ...
          source: 'unified-analysis-engine'
        });
      }
    }
    ...
    return {
      summary: { totalFiles: Object.keys(atomsByFile).length, totalIssues: criticalCount + highCount + mediumCount, criticalCount, highCount, mediumCount, lowCount, tunnelVisionIntegrated: true },
      topRiskFiles: filteredRisks.slice(0, 10),
      recommendation: criticalCount > 0 ? '🚨 Critical issues detected - Review high-risk files' : '✓ Risk levels acceptable'
    };
  } catch (error) {
    logger.error(`Error in get_risk_assessment: ${error.message}`);
    return { summary: { totalFiles: 0, totalIssues: 0, criticalCount: 0, highCount: 0, mediumCount: 0, lowCount: 0 }, topRiskFiles: [], recommendation: 'Error accessing project data', error: error.message };
  }
}
