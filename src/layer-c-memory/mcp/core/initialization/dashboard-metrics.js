...
function createExtendedMetricsBase(startupTelemetry, db) {
  return {
    totalAtoms: 0,
    typeSummary: '',
    callableSummary: '',
...
function populatePreReconciliationExtendedMetrics(metrics, db) {
  loadAtomInventory(metrics, db);
  loadGraphMetrics(metrics, db);
  loadPhysicsMetrics(metrics, db);
}

async function populatePostReconciliationExtendedMetrics(metrics, {
  projectPath,
  db,
  repo,
  isFinal,
  ...
  metrics.updateSurface = buildBootstrapUpdateSurface({
    ...
    db,
    repo,
    liveRowSyncSummary: metrics.liveRowSyncSummary,
    fileUniverseSummary: compilerDiagnostics?.fileUniverseGranularity || {},
    phase2PendingFiles: metrics.phase2PendingFiles,
    mcpSessionSummary: sessionSummary
  });
  metrics.statusPayload = buildBootstrapStatusPayload({
    projectPath,
    db,
    liveRowSyncSummary: metrics.liveRowSyncSummary,
    fileUniverseSummary: compilerDiagnostics?.fileUniverseGranularity || null,
    phase2PendingFiles: metrics.phase2PendingFiles,
    metrics: {
      totalAtoms: metrics.totalAtoms,
      callLinks: metrics.callLinks,
      liveCoverageRatio: metrics.liveCoverageRatio,
      conceptualGroups: metrics.conceptualGroups,
      conceptualRawGroups: metrics.conceptualRawGroups,
      conceptualActionableRatio: metrics.conceptualActionableRatio,
...
async function fetchExtendedMetrics(projectPath, db, repo, isFinal, startupTelemetry = null) {
  const metrics = createExtendedMetricsBase(startupTelemetry, db);

  try {
    populatePreReconciliationExtendedMetrics(metrics, db);
    ...
    await populatePostReconciliationExtendedMetrics(metrics, {
      projectPath,
      db,
      repo,
      isFinal,
...
export {
  ...
  loadAtomInventory,
  loadConceptualDuplicateMetrics,
  loadGraphMetrics,
  loadIssueMetrics,
  loadPhase2Metrics,
  loadPhysicsMetrics,
  loadStructuralDuplicateMetrics
};
