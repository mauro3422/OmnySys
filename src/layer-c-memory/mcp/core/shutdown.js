...
async function closeManagedResources(serverInstance) {
...
function logShutdownMode(serverInstance) {
...
export async function performServerShutdown(serverInstance) {
  ...
  try {
    await stopLlmServers();
    await closeManagedResources(serverInstance);
    logShutdownMode(serverInstance);
  } catch (error) {
