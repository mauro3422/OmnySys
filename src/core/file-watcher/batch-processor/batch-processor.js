...
export class SmartBatchProcessor {
  ...
  _evaluateBatchMode() {
    const now = Date.now();
    const recentChanges = Array.from(this.changeBuffer.values())
      .filter(c => now - c.timestamp < 1000);
    
    const changesPerSecond = recentChanges.length;
    
    if (changesPerSecond > BATCH_CONFIG.MASS_CHANGE_THRESHOLD) {
      if (this.state !== BatchState.ACCUMULATING) {
        this._setState(BatchState.ACCUMULATING);
        
        if (this.options.verbose) {
          logger.info(`🚀 Batch mode activated: ${changesPerSecond} changes/second`);
        }
      }
    }
  }
