...
function loadConceptualLocalAtoms(repo, normalizedFilePath, minLinesOfCode) {
...
function shouldSkipConceptualAtom(normalizedFilePath, localAtom, isLowSignalNameFn) {
...
function loadConceptualDuplicateRows(repo, normalizedFilePath, fingerprint) {
...
function loadLocalStructuralHash(repo, atomId) {
    return repo.db.prepare(`
...
function buildConceptualFinding(localAtom, structuralVariants, testabilitySeverity) {
...
async function runConceptualDuplicateGuard(repo, normalizedFilePath, options) {
    ...
    try {
        ...
        for (const localAtom of localAtoms) {
            if (shouldSkipConceptualAtom(normalizedFilePath, localAtom, isLowSignalName)) {
                continue;
            }
            ...
            findings.push(buildConceptualFinding(localAtom, structuralVariants, testabilitySummary.severity));
