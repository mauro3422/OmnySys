...
- Duplicate policy consumption is now standardized too: the shared compiler API owns duplicate scope SQL, atom normalization, eligibility filtering, and mode selection, so FileWatcher guards and MCP semantic queries reuse the same policy instead of only sharing the raw DNA key builder.
- Impact/topology consumption is now standardized too: dependency APIs expose a canonical `getFileImpactSummary()` so MCP validation and shared analysis stop recomputing direct/transitive dependents and fragility warnings by hand.
- File discovery now has canonical compiler entrypoints too: shared compiler helpers wrap project scanning so MCP/runtime paths stop reimplementing recursive filesystem walks by hand.
